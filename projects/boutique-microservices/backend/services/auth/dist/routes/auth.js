...
router.post('/register', async (req, res) => {
    try {
        ...
        const result = await (0, connection_1.query)('INSERT INTO users (email, password_hash, first_name, last_name, role) VALUES ($1, $2, $3, $4, $5) RETURNING id, email, first_name, last_name, role, created_at, updated_at', [email, hashedPassword, firstName, lastName, 'customer']);
...
router.post('/login', async (req, res) => {
    try {
        ...
        const result = await (0, connection_1.query)('SELECT id, email, password_hash, first_name, last_name, role, created_at, updated_at FROM users WHERE email = $1', [email]);
        if (result.rows.length === 0) {
...
router.get('/me', async (req, res) => {
    ...
    try {
        const result = await (0, connection_1.query)('SELECT id, email, first_name, last_name, role FROM users WHERE id = $1', [userId]);
        if (result.rows.length === 0) {
